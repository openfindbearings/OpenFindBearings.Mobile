# OpenFindBearings.Mobile BFF 设计 v1.6.1
...
| v1.6.1 | 2026-09-19 | withdraw / delete-application 代理错误透传修复：原 `PostVoidAsync` 把 API 的 400 守卫原因（"仅待审核可撤回 / 仅被驳回可删除 / 无权操作 / 渠道不支持自助"）全部吞成固定"撤回失败/删除失败"，误导排障（曾据此误判为部署时差）。改 `PostWithResultAsync<object>` 按状态码与 ProblemDetails detail 透传，前端 toast 显示真实原因。 |
